...
def api_preventivas():
    try:
        ...
        for idx, p in enumerate(planos, 1):
            ...
            # já declara (ex: "... - NOTURNO"), senão o motor nunca dá o bônus de turno
            # certo pra ninguém noturno (a hora-padrão de 8h é sempre diurna).
            # exigir_turno só vale quando o nome do plano é explícito sobre o turno —
            # pra planos sem essa informação, turno continua só bônus de score (nunca
            # vira filtro obrigatório baseado num hora_ref chutado).
